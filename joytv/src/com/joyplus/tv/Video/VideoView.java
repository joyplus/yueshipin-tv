...
public class VideoView extends SurfaceView implements MediaController.MediaPlayerControl {
	...
	public void setVideoLayout(int layout, float aspectRatio) {
		...
		String mlog = null;
		mlog.format("VIDEO: %dx%dx%f, Surface: %dx%d, LP: %dx%d, Window: %dx%dx%f", mVideoWidth, mVideoHeight, mVideoAspectRatio, mSurfaceWidth, mSurfaceHeight, lp.width, lp.height, windowWidth, windowHeight, windowRatio);
		Log.d(TAG,mlog);
		mVideoLayout = layout;
...
//			mVideoAspectRatio = mp.getVideoAspectRatio();
    ...
    public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
			...
			logString.format("Error: %d, %d", framework_err, impl_err);
			Log.d(TAG,logString);
			mCurrentState = STATE_ERROR;
			mTargetState = STATE_ERROR;
			if (mMediaController != null)
		...
		public boolean onInfo(MediaPlayer mp, int what, int extra) {
			...
			if (mOnInfoListener != null) {
				...
				if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START)
					...
					String logString = null;
					logString.format("onInfo: (%d, %d)", what, extra);
					Log.d(TAG,logString);
				}
			}
			return true;
