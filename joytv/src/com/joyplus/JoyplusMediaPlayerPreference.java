...
public class JoyplusMediaPlayerPreference extends Dialog{
	...
	private Gallery gallery,gallery_zm;
	...
	private void initView() {
		...
		gallery_zm = (Gallery) findViewById(R.id.gallery_zimu);
	}
	
	public void setDefinationGalleryAdapter(ArrayList<Integer> list,int defination){
		
		definationStrings.clear();
		if(list != null){
			definationStrings = list;
		}
		gallery.setAdapter(new DefinationAdapter(mContext, definationStrings));
		if(definationStrings.size() > 0 && defination < definationStrings.size()){
			
			gallery.setSelection(definationStrings.indexOf(defination));
		}
