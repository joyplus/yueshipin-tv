...
public class CharsetUtil {
    ...
    static {
        ...
        if(Debug)Log.d(TAG,"Character sets which support decoding: "
                        + decodingSupported);
        if(Debug)Log.d(TAG,"Character sets which support encoding: "
                        + encodingSupported);
    }
    ...
    public static java.nio.charset.Charset getCharset(String charsetName) {
        ...
        try {
            ...
            if(Debug)Log.d(TAG,"Illegal charset " + charsetName + ", fallback to " + defaultCharset + ": " + e);
            // Use default charset on exception
            return java.nio.charset.Charset.forName(defaultCharset);
        } catch (UnsupportedCharsetException ex) {
        	if(Debug)Log.d(TAG,"Unsupported charset " + charsetName + ", fallback to " + defaultCharset + ": " + ex);
            // Use default charset on exception
            return java.nio.charset.Charset.forName(defaultCharset);
        }
